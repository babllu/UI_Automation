...
public class optimzed_Addtocart {
	...
	public static void main(String[] args) {
		...
		addItems(driver, reqitems); 
	}

	public static void addItems(WebDriver driver, String[] reqitems) {
		...
		for (int i = 0; i < products.size(); i++) {
			...
			// check whether extracted product name is present in arrayList or not.
			...
			if (itemsreqrd.contains(finalname)) {
				...
				List<WebElement> select = driver.findElements(By.xpath("//div[@class='product-action']/button"));
				select.get(i).click();
